...
// Predefined RGB color values for Yellow, Pink, Yellow-Pink
// {R, G, B}
const int PREDEFINED_COLORS[3][3] = {
  {255, 255, 0},   // 0: Yellow
  {255, 105, 180}, // 1: Pink (Hot Pink)
  {255, 160, 90}   // 2: Yellow-Pink (Peach/Salmon-like)
};
// Note: User can adjust these RGB values as needed.

// Helper function to parse hex color string to R, G, B values - NO LONGER NEEDED for setPlantLedColor with predefined options
...
// plantIndex: 0-4
// colorOptionIndex: 0 for Yellow, 1 for Pink, 2 for Yellow-Pink
void setPlantLedColor(int plantIndex, int colorOptionIndex, int brightnessPercent) {
    int r, g, b;

    if (colorOptionIndex >= 0 && colorOptionIndex < 3) {
      r = PREDEFINED_COLORS[colorOptionIndex][0];
      g = PREDEFINED_COLORS[colorOptionIndex][1];
      b = PREDEFINED_COLORS[colorOptionIndex][2];
    } else {
      // Default to off or a specific color if index is invalid
      r = 0; g = 0; b = 0; // Off
    }

    // Adjust RGB values by brightness (0-24 from profile)
