...
struct PlantAssignment {
  ...
  int colorOption;    // 0: Yellow, 1: Pink, 2: Yellow-Pink
};
...
const int PatteCapteur1 = 4;
const int PatteCapteur2 = 5;
const int PatteCapteur3 = 6;
const int PatteCapteur4 = 7;
const int PatteCapteur5 = 15;
...
const int PattePompe1 = 17;
const int PattePompe2 = 18;
const int PattePompe3 = 8;
const int PattePompe4 = 3;
const int PattePompe5 = 46;

// New LED Pin Definitions (Yellow and Pink per plant)
// Plant 1
const int LED_J_PIN_1 = 43; // Jaune
const int LED_P_PIN_1 = 41; // Rose
// Plant 2
const int LED_J_PIN_2 = 44;
const int LED_P_PIN_2 = 40;
// Plant 3
const int LED_J_PIN_3 = 1;
const int LED_P_PIN_3 = 39;
// Plant 4
const int LED_J_PIN_4 = 2;
const int LED_P_PIN_4 = 38;
// Plant 5
const int LED_J_PIN_5 = 42;
const int LED_P_PIN_5 = 37;
...
// Arrays to hold the pin numbers for easier access in loops
const int led_j_pins[] = {LED_J_PIN_1, LED_J_PIN_2, LED_J_PIN_3, LED_J_PIN_4, LED_J_PIN_5};
const int led_p_pins[] = {LED_P_PIN_1, LED_P_PIN_2, LED_P_PIN_3, LED_P_PIN_4, LED_P_PIN_5};

void setupLedPwm() {
    // Attach all 10 LED pins to PWM
    for (int i = 0; i < 5; i++) {
        ledcAttach(led_j_pins[i], PWM_FREQ, PWM_RESOLUTION);
        ledcAttach(led_p_pins[i], PWM_FREQ, PWM_RESOLUTION);
    }
}

// Function to set the color and brightness for a plant's LED
// plantIndex: 0-4
// colorOptionIndex: 0 for Yellow, 1 for Pink, 2 for Yellow-Pink (both)
void setPlantLedColor(int plantIndex, int colorOptionIndex, int brightnessPercent) {
    if (plantIndex < 0 || plantIndex >= 5) return; // Safety check

    // Calculate brightness duty cycle (0-255) from profile setting (0-24)
    int dutyCycle = map(brightnessPercent, 0, 24, 0, 255);
    ...
    switch (colorOptionIndex) {
        ...
        case 2: // Jaune-Rose
            ...
            pinkDuty = dutyCycle;
            break;
        default: // Off or invalid option
            break;
    }

    // Write the duty cycle to the correct plant's LEDs
    ledcWrite(led_j_pins[plantIndex], yellowDuty);
    ledcWrite(led_p_pins[plantIndex], pinkDuty);
}
...
void Plante1() {
  ...
  if (plantsA[0].profileIndex != -1 && !Manuel) { // Vérifie si un profil est assigné et que le mode manuel n'est pas actif
    ...
    // Utiliser le nouveau système de seuils
    int mappedTarget = seuils[targetHumiditySetting];
...
<body>
  ...
  for (int i = 0; i < 5; ++i) {
    ...
    // Replace color picker with dropdown
    html += "<div class='color-option-container'>Couleur: <select id='colorOptionSelect-" + String(i) + "'>";
    html += "<option value='0'>Jaune</option>";
    html += "<option value='1'>Rose</option>";
    html += "<option value='2'>Jaune-Rose</option>";
    html += "</select></div>";
...
html += R"(
    ...
    <script>
        ...
        const colorNames = ["Jaune", "Rose", "Jaune-Rose"]; // Helper for displaying color names
        ...
        function updatePlantDisplay() { for(let i=0; i<5; i++) { if(plants_data[i]) { document.getElementById('profileSelect-'+i).value = plants_data[i].profileIndex; document.getElementById('colorOptionSelect-'+i).value = plants_data[i].colorOption !== undefined ? plants_data[i].colorOption : 0; updatePlantDetails(i); }}}
        function updatePlantDetails(plantIndex) { const p_assign = plants_data[plantIndex]; const detDiv = document.getElementById('details-'+plantIndex); if(p_assign && p_assign.profileIndex >=0 && p_assign.profileIndex < profiles_data.length) { const prof = profiles_data[p_assign.profileIndex]; if(prof.name && prof.name.trim()!=='') { let dets='<div class="detail-item">💧 Hum: '+prof.humidity+'</div><div class="detail-item">💡 Lum: '+prof.brightness+'</div>'; if(p_assign.colorOption !== undefined && p_assign.colorOption >= 0 && p_assign.colorOption < colorNames.length) { dets+='<div class="detail-item">🎨 Coul: '+colorNames[p_assign.colorOption]+'</div>';} detDiv.innerHTML=dets; detDiv.style.display='block'; } else {detDiv.style.display='none';} } else {detDiv.style.display='none';}}
        function assignProfile(plantIdx) { const profIdx = parseInt(document.getElementById('profileSelect-'+plantIdx).value); const colorOpt = parseInt(document.getElementById('colorOptionSelect-'+plantIdx).value); const formData = new FormData(); formData.append('plant', plantIdx); formData.append('profileIndex', profIdx); formData.append('colorOption', colorOpt); fetch('/assign',{method:'POST',body:formData}).then(r=>r.text()).then(d=>{ if(d==='OK'){ if(!plants_data[plantIdx]) plants_data[plantIdx]={}; plants_data[plantIdx].profileIndex=profIdx; plants_data[plantIdx].colorOption=colorOpt; updatePlantDetails(plantIdx); updateStats(); showMessage('✅ Plante '+(plantIdx+1)+' configurée!', 'success');} else {showMessage('❌ Erreur attribution', 'error');}}).catch(e => showMessage('❌ Erreur: '+e, 'error'));}
        function updateStats() { const statsDiv = document.getElementById('stats'); let assigned=0; let html=''; for(let i=0; i<5; i++) { const p_assign=plants_data[i]; html+='<div class="stat-item"><strong>Plante '+(i+1)+':</strong> '; if(p_assign && p_assign.profileIndex >=0 && p_assign.profileIndex < profiles_data.length){ const prof=profiles_data[p_assign.profileIndex]; if(prof.name && prof.name.trim()!==''){ html+=prof.name+' (H:'+prof.humidity+', L:'+prof.brightness+')'; if(p_assign.colorOption !== undefined && p_assign.colorOption >= 0 && p_assign.colorOption < colorNames.length) {html+=' '+colorNames[p_assign.colorOption];} assigned++; } else {html+='Profil vide';}} else {html+='Non assignée';} html+='</div>';} html='<div class="summary">📈 '+assigned+'/5 plantes configurées</div>'+html; statsDiv.innerHTML=html;}
...
void handleAssign() {
  if (server.hasArg("plant") && server.hasArg("profileIndex") && server.hasArg("colorOption")) { // Changed "color" to "colorOption"
    int plantIndex = server.arg("plant").toInt(); // Expecting 0-4 from JavaScript
    int profileIndex = server.arg("profileIndex").toInt();
    int colorOpt = server.arg("colorOption").toInt(); // Expecting 0, 1, or 2

    if (plantIndex >= 0 && plantIndex < 5) {
      plantsA[plantIndex].profileIndex = profileIndex;
      if (colorOpt >= 0 && colorOpt <= 2) { // Validate colorOption
        plantsA[plantIndex].colorOption = colorOpt;
      } else {
        plantsA[plantIndex].colorOption = 0; // Default to Yellow if invalid option received
      }

      savePlants(); // Save the updated plantsA array to EEPROM

      Serial.printf("Plante %d (JS index %d) assignée au profil %d, Option Couleur: %d\n",
                    plantIndex + 1, plantIndex, profileIndex, plantsA[plantIndex].colorOption);
      server.send(200, "text/plain", "OK");
    } else {
      server.send(400, "text/plain", "Index de plante invalide.");
    }
  } else {
    server.send(400, "text/plain", "Paramètres manquants. 'plant', 'profileIndex', et 'colorOption' sont requis.");
  }
}
...
void handleGetPlants() {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    json += "\"colorOption\":" + String(plantsA[i].colorOption); // Send integer colorOption
    json += "}";
  }
  json += "]";
  server.send(200, "application/json", json);
...
void loadData() {
  ...
  for (int i = 0; i < 5; i++) {
    ...
    if (nameIsEmpty || plantsA[i].profileIndex < -1 || plantsA[i].profileIndex >= 50) {
      ...
      plantsA[i].colorOption = 0; // Default to Yellow if plant assignment is reset
    }
    // Initialize colorOption if it seems uninitialized (e.g. new field or corrupted EEPROM)
    // Valid options are 0, 1, 2. If it's outside this, default to 0.
    if (plantsA[i].colorOption < 0 || plantsA[i].colorOption > 2) {
        plantsA[i].colorOption = 0; // Default to Yellow (index 0)
    }
  }
}
